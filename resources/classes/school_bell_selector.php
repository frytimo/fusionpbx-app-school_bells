...
class school_bell_selector {
	...
	/**
	 * Creates a new instance of school bell selector object
	 * @param $time12h (Optional default false). Sets the time format to use 12 hour or 24 hour clock
	 */
	...
	private function _option_string($option_item, $draw_item, $selected_item) {
		if ($option_item == $selected_item) {
			return "<option selected value='" . escape($option_item) . "'>" . escape($draw_item) . "</option>\n";
		}
		return "<option value='" . escape($option_item) . "'>" . escape($draw_item) . "</option>\n";
	}

	private function _draw_selected($selector_name, $range_array, $selected) {
		$selector_text = "<select name = '" . escape($selector_name) . "' id = '" . escape($selector_name) . "' class='formfld'>\n";
